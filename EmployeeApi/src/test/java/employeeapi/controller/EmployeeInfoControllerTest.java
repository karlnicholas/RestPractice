...
//        sparseEmployeeDetailItems.add(sparseEmployeeDetailItem);
    ...
    public void testGet() throws Exception {
        ...
        .andExpect(jsonPath("$.content[0].name", is("Karl")))
    ...
    public void testGetProjects() throws Exception {
        ...
        .andExpect(jsonPath("$.content[0].projectName", is("Test Project")))
        .andExpect(jsonPath("$.size", is(20)))
        .andExpect(jsonPath("$.totalElements", is(1)))
        .andExpect(jsonPath("$.totalPages", is(1)))
        .andExpect(jsonPath("$.number", is(0)))
...
//            .andDo(print())    
            .andExpect(status().isOk())    
            .andExpect(header().string(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE + ";charset=UTF-8"))
            .andExpect(jsonPath("$.employeeAddressItem.empId", is(1)))  
            .andExpect(jsonPath("$.employeeAddressItem.address1", is("Address 1")))
            .andExpect(jsonPath("$.employeeAddressItem.address2", is("Address 2")))
            .andExpect(jsonPath("$.employeeAddressItem.address3", is("Address 3")))
            .andExpect(jsonPath("$.employeeAddressItem.address4", is("Address 4")))
            .andExpect(jsonPath("$.employeeAddressItem.state", is("AZ")))
            .andExpect(jsonPath("$.employeeAddressItem.country", is("US")))
            .andExpect(jsonPath("$.employeeDetailItem.name", is("Karl Nicholas")))
            .andExpect(jsonPath("$.employeeDetailItem.role", is("4")))
            .andExpect(jsonPath("$.employeeDetailItem.salary", is(100000.00)))
            .andExpect(jsonPath("$.employeeDetailItem.roleDescription", is("Technical Analyst")))
            .andExpect(jsonPath("$.projectItems[0].projectId", is(1)))
            .andExpect(jsonPath("$.projectItems[0].projectName", is("Test Project Name")))
            .andExpect(jsonPath("$.projectItems[0].techstack", is("Test Project Techstack")))
